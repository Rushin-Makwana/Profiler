...
public class ProfilerAgent {
    ...
    public static void premain(String agentArgs, Instrumentation inst) {
        ...
        try {
            // Append agent jar to bootstrap classpath so bootstrap classes (like
            // java.util.*)
            ...
            // Enable Byte Buddy experimental support to allow instrumenting classes in
            // newer JVMs (like Java 25+)
            System.setProperty("com.ag.profiler.bytebuddy.experimental", "true");

            // Use reflection to load AgentInstaller to prevent premature loading of Byte
            // Buddy
            // classes by the application class loader before they are added to the
            // bootstrap path.
            Class<?> installerClass = Class.forName("com.ag.profiler.AgentInstaller");
            installerClass.getMethod("install", String.class, Instrumentation.class, String.class).invoke(null,
                    agentArgs, inst, jarPath);
        } catch (Exception e) {
